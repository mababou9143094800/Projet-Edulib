...
if (!$resource) {
    ...
    echo '<div class="container section"><div class="alert alert--error">Cette ressource n\'existe pas ou a été supprimée.</div><a href="/frontend/resources.php" class="btn btn--ghost mt-2">Retour aux ressources</a></div>';
...
if ($resource['status'] === 'draft' && !$is_mine && !is_admin()) {
    ...
    include __DIR__ . '/../backend/includes/header.php';
    echo '<div class="container section"><div class="alert alert--error">Cette ressource n\'est pas encore publiée.</div><a href="/frontend/resources.php" class="btn btn--ghost mt-2">Retour aux ressources</a></div>';
    include __DIR__ . '/../backend/includes/footer.php';
    exit;
}
...
$img_stmt = db()->prepare('SELECT filename FROM resource_images WHERE resource_id = ? ORDER BY sort_order ASC');
$img_stmt->execute([$id]);
...
<div class="container section">
  ...
  <nav class="breadcrumb" aria-label="Fil d'Ariane">
    ...
    <a href="/frontend/resources.php">Ressources</a>
    <span class="breadcrumb__sep" aria-hidden="true">›</span>
    <a href="/frontend/resources.php?categorie=<?= $resource['categorie_id'] ?>"><?= e($resource['cat_nom']) ?></a>
    <span class="breadcrumb__sep" aria-hidden="true">›</span>
    ...
    <div class="flex gap-1 mt-2 flex-wrap items-center">
      ...
      <?php if ($resource['status'] === 'published'): ?>
        <form method="post" action="/backend/vote.php">
          ...
          <?php else: ?>
            <a href="/frontend/login.php" class="btn btn--sm btn--outline">▲ <?= $vote_count ?> vote<?= $vote_count !== 1 ? 's' : '' ?></a>
          <?php endif; ?>
        </form>

        <?php if ($user): ?>
          <form method="post" action="/backend/favorite.php">
            <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
            <input type="hidden" name="resource_id" value="<?= $resource['id'] ?>">
      ...
      <?php if ($is_mine || is_admin()): ?>
        <a href="/backend/edit-resource.php?id=<?= $resource['id'] ?>" class="btn btn--ghost btn--sm">Modifier</a>
        <form method="post" action="/backend/delete-resource.php" style="display:inline;"
  ...
  <?php if ($images): ?>
    <div class="image-gallery" role="region" aria-label="Galerie d'images">
      <?php foreach ($images as $filename): ?>
        <div class="image-gallery__item">
          <img src="/uploads/resources/<?= e($filename) ?>"
               alt="Illustration de la ressource" loading="lazy">
        </div>
      <?php endforeach; ?>
    </div>
  <?php endif; ?>
  ...
  <section id="comments" class="comments-section">
    ...
    <?php if ($user && $resource['status'] === 'published'): ?>
      <form method="post" action="/backend/comment.php" class="comment-form mb-3">
        <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
        <input type="hidden" name="resource_id" value="<?= $resource['id'] ?>">
    ...
    <?php elseif (!$user): ?>
      <p class="text-muted text-sm mb-3"><a href="/frontend/login.php">Connectez-vous</a> pour laisser un commentaire.</p>
    <?php endif; ?>
          ...
          <div class="comment" id="comment-<?= $c['id'] ?>">
            <div class="comment__header">
              ...
              <?php if ($user && ($c['user_id'] === $user['id'] || is_admin())): ?>
                <form method="post" action="/backend/comment.php" style="margin-left:auto;">
                  <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
            ...
            <?php if ($user && $resource['status'] === 'published'): ?>
              <details class="reply-toggle">
                ...
                <form method="post" action="/backend/comment.php" class="comment-form mt-1">
                  <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
                  <input type="hidden" name="resource_id" value="<?= $resource['id'] ?>">
  ...
  <hr class="divider">
  <a href="/frontend/resources.php" class="btn btn--ghost btn--sm">&larr; Retour aux ressources</a>
